...
const handleDeletePlotOwnerPrivate = (_event: Event, plotOwnerPrivate: PlotOwnerPrivate) => {
    ...
    if (currentPlotForPrivateList.value.plotOwnerPrivate) {
        const index = currentPlotForPrivateList.value.plotOwnerPrivate.findIndex(
            p => p.id === plotOwnerPrivate.id
        );
        if (index !== -1) {
...
<template>
    <div class="p-6">
                    ...
                    <div v-for="plot in plots" :key="plot.id"
                        ...
                        <div class="grid grid-cols-6 gap-3">
                            ...
                            <div class="mt-3">
                                ...
                                <p :class="`text-base font-bold ${plot.connectionEntity
                            ...
                            <div class="mt-3">
                                ...
                                <div class="flex items-center gap-2">
                                    <i :class="plot.plotOwner?.name === 'Wł. prywatny' ? 'pi pi-user' : 'pi pi-building-columns'"
                                        class="text-surface-400 text-sm"></i>
                                    <p class="text-sm text-surface-700 dark:text-surface-300 m-0">
                            ...
                            <div class="flex flex-col items-end gap-0 ">
                                ...
                                <div class="flex ">
                                    ...
                                    <Button :icon="plot.connectionEntity ? 'pi pi-star-fill' : 'pi pi-star'"
                                        :severity="plot.connectionEntity ? 'warning' : 'secondary'" text rounded
                                        :disabled="isReadonly"
                                        :class="plot.connectionEntity ? 'text-orange-600 dark:text-orange-400' : ''"
                                        @click="handleToggleConnectionEntity(plot)"
                                        :title="plot.connectionEntity ? 'Usuń oznaczenie podmiotu przyłączanego' : 'Oznacz jako podmiot przyłączany'" />
                                    <!-- Ikona otwierania dialogu PlotOwnerPrivate -->
                                    <Button icon="pi pi-user-plus" severity="primary" text rounded
                                        :disabled="isPlotOwnerPrivateIconDisabled(plot)"
                                        @click="handleAddPlotOwnerPrivate(plot)" title="Dodaj właściciela prywatnego" />
                                ...
                                <div class="flex">
                                    <Button icon="pi pi-pencil" severity="secondary" text rounded :disabled="isReadonly"
                                        @click="handleEditPlot(plot)" title="Edytuj działkę" />
                                    <Button icon="pi pi-trash" severity="danger" text rounded :disabled="isReadonly"
                                        @click="(event) => handleDeletePlot(event, plot)" title="Usuń działkę" />
                                </div>
                            </div>
                        </div>
                        <!-- Napis o właścicielach prywatnych dla tej działki -->
                        <div v-if="plotHasPrivateOwners(plot)"
                            class="mt-4 pt-4 border-t border-surface-200 dark:border-surface-700">
                            <p
                                :class="`text-sm font-semibold uppercase m-0 ${getPlotOwnerPrivateStatusClassesForPlot(plot)}`">
        ...
        <PlotDialog v-model:visible="plotDialogVisible" :plot="editingPlot" :isReadonly="isReadonly"
            @save="handleSavePlot" />

        <!-- Dialog do dodawania PlotOwnerPrivate -->
        <PlotOwnerPrivateDialog v-model:visible="plotOwnerPrivateDialogVisible" :plot="currentPlotForPrivate"
            :editingPlotOwnerPrivate="editingPlotOwnerPrivate" :isReadonly="isReadonly"
            @save="handleSavePlotOwnerPrivate"
            @update:visible="(val) => { plotOwnerPrivateDialogVisible = val; if (!val) editingPlotOwnerPrivate = null; }" />

        <!-- Dialog z listą PlotOwnerPrivate -->
        <PlotOwnerPrivateListDialog v-model:visible="plotOwnerPrivateListDialogVisible"
            :plot="currentPlotForPrivateList" :isReadonly="isReadonly" @edit="handleEditPlotOwnerPrivate"
