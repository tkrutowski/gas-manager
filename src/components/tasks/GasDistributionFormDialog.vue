...
<template>
  <Dialog
    ...
    modal
    header="Dodaj Nową Jednostkę Zlecającą"
    :style="{ width: '600px' }"
    @hide="handleClose"
  >
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2"> Nr rozdziału </label>
        <InputText
          v-model="formData.distributionNumber"
          class="w-full bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 text-surface-700 dark:text-surface-300"
        />
      </div>
      <div>
        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2"> Przedstawiciel </label>
        <InputText
