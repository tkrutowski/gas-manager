<script setup lang="ts">
  ...
  const defaultFormData = (): Partial<DesignerTraffic> => ({
    name: '',
    lastName: '',
    phones: [],
    emails: [],
  ...
  const formData = ref<Partial<DesignerTraffic>>(defaultFormData());
    ...
    newVal => {
      if (newVal) {
        if (props.designerTraffic) {
          formData.value = {
            ...defaultFormData(),
            ...props.designerTraffic,
            phones: props.designerTraffic.phones ? [...props.designerTraffic.phones] : [],
            emails: props.designerTraffic.emails ? [...props.designerTraffic.emails] : [],
          };
        } else {
    ...
    try {
      ...
      if (isEditMode.value && props.designerTraffic) {
        const updated = designerTrafficStore.updateDesignerTraffic(
          ...
          cleanedData as Partial<Omit<DesignerTraffic, 'id' | 'createdAt'>>
        );
        if (updated) {
        ...
        const created = designerTrafficStore.addDesignerTraffic(
          cleanedData as Omit<DesignerTraffic, 'id' | 'createdAt' | 'updatedAt'>
        );
