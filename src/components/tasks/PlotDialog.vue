...
<template>
  <Dialog
    ...
    :header="isEditMode ? 'Edytuj działkę' : 'Dodaj nową działkę'"
    class="p-fluid"
  >
    <div class="space-y-4">
      <!-- NR DZIAŁKI -->
      <div>
        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2"> NR DZIAŁKI </label>
        <InputText v-model="plotNumber" placeholder="np. 197/1" class="w-full" :disabled="isReadonly || isEditMode" />
      </div>

      <!-- DATA ZŁOŻENIA -->
      <div>
        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2"> DATA ZŁOŻENIA </label>
        <DatePicker
      ...
      <!-- DATA OTRZYMANIA -->
      <div>
        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2"> DATA OTRZYMANIA </label>
        <DatePicker
          v-model="receiptDate"
          :disabled="isReceiptDateDisabled"
          :inputReadonly="true"
          :manualInput="false"
          showButtonBar
          showIcon
      ...
      <!-- WŁAŚCICIEL -->
      <div>
        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2"> WŁAŚCICIEL </label>
        <Select
          v-model="plotOwner"
      ...
      <!-- DODATKOWE INFO -->
      <div>
        <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2"> DODATKOWE INFO </label>
        <Textarea v-model="info" placeholder="Dodatkowe informacje" rows="3" class="w-full" :disabled="isReadonly" />
      </div>
    </div>
