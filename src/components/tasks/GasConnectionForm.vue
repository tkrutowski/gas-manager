...
import { UserIcon, MapPinIcon, DocumentTextIcon, CurrencyDollarIcon, UserGroupIcon, InformationCircleIcon } from '@heroicons/vue/24/outline';
...
<template>
  <div class="min-h-screen bg-surface-0 dark:bg-surface-950 text-gray-900 dark:text-white p-6">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-1 gap-4">
            <div>
              ...
              <div class="flex gap-2">
                <AutoComplete v-model="formData.customer" :suggestions="customerSuggestions" @complete="searchCustomers"
                  :placeholder="'Wybierz zleceniodawcę...'" class="flex-1"
                  :class="{ 'border-red-500': errors.customer }" @item-select="e => handleCustomerSelect(e.value)"
                  dropdown forceSelection :optionLabel="(customer: Customer) => getCustomerDisplayName(customer)">
                  <template #option="{ option }">
                    <div>{{ getCustomerDisplayName(option) }}</div>
                  </template>
                </AutoComplete>
                <SecondaryButton type="button" @click="handleCustomerDetails" :disabled="!formData.customer"
                  icon="pi pi-info-circle" title="Pokaż szczegóły zleceniodawcy" />
        ...
        <div class="bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
              ...
              <div class="flex gap-2">
                <AutoComplete v-model="formData.endCustomer" :suggestions="endCustomerSuggestions"
                  @complete="searchEndCustomers" :placeholder="'Wybierz klienta...'" class="flex-1"
                  :class="{ 'border-red-500': errors.endCustomer }" @item-select="e => handleEndCustomerSelect(e.value)"
                  dropdown forceSelection :optionLabel="(customer: Customer) => getCustomerDisplayName(customer)">
                  <template #option="{ option }">
                    <div>{{ getCustomerDisplayName(option) }}</div>
                  </template>
                </AutoComplete>
                <SecondaryButton type="button" @click="handleEndCustomerDetails" :disabled="!formData.endCustomer"
                  icon="pi pi-info-circle" title="Pokaż szczegóły klienta" />
            ...
            <div>
              ...
              <InputText v-model="addressCity" :class="{ 'border-red-500': errors.addressCity }"
                class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
            ...
            <div>
              ...
              <InputText v-model="addressStreet" :class="{ 'border-red-500': errors.addressStreet }"
                class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
        ...
        <div class="bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <InputText v-model="formData.taskNo" :class="{ 'border-red-500': errors.taskNo }"
                class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
            ...
            <div>
              ...
              <DatePicker v-model="formData.contractDate" dateFormat="mm/dd/yy"
                :class="{ 'border-red-500': errors.contractDate }" class="w-full" showIcon />
            ...
            <div>
              ...
              <InputText v-model="formData.contractNo" :class="{ 'border-red-500': errors.contractNo }"
                class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
            ...
            <div>
              ...
              <DatePicker v-model="formData.conditionDate" dateFormat="mm/dd/yy"
                :class="{ 'border-red-500': errors.conditionDate }" class="w-full" showIcon />
            ...
            <div>
              ...
              <DatePicker v-model="formData.wsgFinalPickupDate" dateFormat="mm/dd/yy"
            ...
            <div>
              ...
              <InputText v-model="formData.conditionNo"
                class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          ...
          <div
            ...
            <div class="grid grid-cols-1 gap-4">
              <div>
                ...
                <div class="flex items-center gap-2">
                  <InputNumber v-model="formData.taskValue" mode="decimal" :min="0"
                    :class="{ 'border-red-500': errors.taskValue }" class="flex-1" />
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  <InputNumber v-model="formData.projectValue" mode="decimal" :min="0"
                    :class="{ 'border-red-500': errors.projectValue }" class="flex-1" />
                  <span class="text-gray-600 dark:text-gray-400">PLN</span>
                </div>
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  <InputNumber v-model="formData.constructionValue" mode="decimal" :min="0"
                    :class="{ 'border-red-500': errors.constructionValue }" class="flex-1" />
                  <span class="text-gray-600 dark:text-gray-400">PLN</span>
                </div>
              ...
              <div>
                ...
                <div class="flex gap-2">
                  <Select v-model="formData.gasDistribution" :options="gasDistributionsStore.activeGasDistributions"
                    placeholder="Wybierz jednostkę..." class="flex-1" optionLabel="name" :filter="true"
                    filterPlaceholder="Szukaj jednostki..." :showClear="true"
          ...
          <div
            ...
            <div class="grid grid-cols-1 gap-4">
              <div>
                ...
                <div class="flex gap-2">
                  <AutoComplete v-model="formData.designer" :suggestions="designerSuggestions"
                    @complete="searchDesigners" :placeholder="'Wybierz projektanta...'" class="flex-1"
                    @item-select="e => handleDesignerSelect(e.value)" dropdown forceSelection
              ...
              <div>
                ...
                <div class="flex gap-2">
                  <AutoComplete v-model="formData.coordinator" :suggestions="coordinatorSuggestions"
                    @complete="searchCoordinators" :placeholder="'Wybierz koordynatora...'" class="flex-1"
                    @item-select="e => handleCoordinatorSelect(e.value)" dropdown forceSelection
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Koordynator
                  projekt</label>
                <div class="flex gap-2">
                  <AutoComplete v-model="coordinatorProjectSearchQuery" :suggestions="coordinatorProjectSuggestions"
                    @complete="searchCoordinatorProjects" :placeholder="'Wybierz koordynatora projektu...'"
                    class="flex-1" @item-select="e => handleCoordinatorProjectSelect(e.value)" dropdown forceSelection
                    :optionLabel="(coordinator: Coordinator) => `${coordinator.name} ${coordinator.lastName}`">
                    <template #option="{ option }">
                      <div>{{ option.name }} {{ option.lastName }}</div>
                    </template>
                  </AutoComplete>
                  <PrimaryButton type="button" @click="handleNewCoordinatorProject"
              ...
              <div>
                ...
                <Textarea v-model="formData.info" rows="4"
                  class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
              </div>
            </div>
          </div>
        ...
        <div class="bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-6">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              ...
              <Popover ref="phaseHelpPopover">
                <div class="p-3" style="max-width: 320px">
                  <p class="text-sm text-surface-700 dark:text-surface-300 leading-relaxed">
                    <strong class="font-semibold">Faza projektu</strong> określa aktualny etap realizacji przyłącza
                    gazowego.
                  </p>
                  <p class="text-sm text-surface-600 dark:text-surface-400 mt-2 leading-relaxed">
                    Wybierz odpowiednią fazę zgodnie z postępem prac:
                  </p>
                  <ul class="text-sm text-surface-600 dark:text-surface-400 mt-2 ml-4 list-disc space-y-1">
                    <li>W przygotowaniu</li>
                    <li>Projekt - przyłącze jest w trakcie projektowania</li>
                    <li>Budowa - przyłącze jest w trakcie budowy</li>
                    <li>Finanse - zakończono budowę i została wystawiona faktura</li>
                    <li>Zakończone - przyłącze jest zakończone i faktura została uregulowana</li>
                    <li>Anulowane - budowa przyłącza została anulowana</li>
                  </ul>
                </div>
              </Popover>
            </h2>
          ...
          <div class="grid grid-cols-1 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Faza<span class="text-primary-400">*</span>
              </label>
              <Select v-model="formData.phase" :options="phaseOptions" placeholder="Wybierz fazę..." class="w-full"
                optionLabel="label" optionValue="value" :filter="true" filterPlaceholder="Szukaj fazy..."
                :class="{ 'border-red-500': errors.phase }" />
              <p v-if="errors.phase" class="text-red-500 text-sm mt-1">{{ errors.phase }}</p>
            </div>
          </div>
        </div>
    ...
    <!-- Dialogs -->
    <CustomerDetailsDialog v-if="showCustomerDetailsDialog" :customer="selectedCustomerForDetails!"
      @close="showCustomerDetailsDialog = false" />
    <CustomerFormDialog v-if="showCustomerFormDialog" @close="showCustomerFormDialog = false"
      @customer-added="handleCustomerAdded" />
    <CustomerDetailsDialog v-if="showEndCustomerDetailsDialog" :customer="selectedCustomerForDetails!"
