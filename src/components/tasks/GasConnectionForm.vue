            ...
            Object.keys(parsed).forEach(key => {
                if (parsed[key] && typeof parsed[key] === 'object' && !Array.isArray(parsed[key])) {
                    Object.keys(parsed[key]).forEach(subKey => {
                        if (parsed[key][subKey] && typeof parsed[key][subKey] === 'string' && parsed[key][subKey].match(/^\d{4}-\d{2}-\d{2}/)) {
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <div class="flex gap-2">
                                <AutoComplete v-model="formData.customer" :suggestions="customerSuggestions"
                                    @complete="searchCustomers" :placeholder="'Wybierz zleceniodawcę...'" class="flex-1"
                                    :class="{ 'border-red-500': errors.customer }"
                                    @item-select="(e) => handleCustomerSelect(e.value)" dropdown forceSelection
                                    :optionLabel="(customer: Customer) => getCustomerDisplayName(customer)">
                                    <template #option="{ option }">
                                        <div>{{ getCustomerDisplayName(option) }}</div>
                                    </template>
                                </AutoComplete>
                                <SecondaryButton type="button" @click="handleCustomerDetails"
                                    :disabled="!formData.customer" icon="pi pi-info-circle"
                                    title="Pokaż szczegóły zleceniodawcy" />
                                <PrimaryButton type="button" @click="handleNewCustomer"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <div class="flex gap-2">
                                <AutoComplete v-model="formData.endCustomer" :suggestions="endCustomerSuggestions"
                                    ...
                                    @item-select="(e) => handleEndCustomerSelect(e.value)" dropdown forceSelection
                                    :optionLabel="(customer: Customer) => getCustomerDisplayName(customer)">
                                    <template #option="{ option }">
                                        <div>{{ getCustomerDisplayName(option) }}</div>
                                    </template>
                                </AutoComplete>
                                <SecondaryButton type="button" @click="handleEndCustomerDetails"
                                    :disabled="!formData.endCustomer" icon="pi pi-info-circle"
                        ...
                        <div>
                            ...
                            <InputText v-model="addressStreet" :class="{ 'border-red-500': errors.addressStreet }"
                                class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
                            <p v-if="errors.addressStreet" class="text-red-500 text-sm mt-1">{{ errors.addressStreet }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <p v-if="errors.contractDate" class="text-red-500 text-sm mt-1">{{ errors.contractDate }}
                            </p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        ...
                        <div>
                            ...
                            <p v-if="errors.conditionDate" class="text-red-500 text-sm mt-1">{{ errors.conditionDate }}
                            </p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">SAP/UP</label>
                            <InputText v-model="formData.sapUpNo"
                                class="w-full bg-white dark:bg-gray-800 border border-surface-200 dark:border-surface-700 text-gray-900 dark:text-white" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Data
                                przyspieszenia</label>
                            <DatePicker v-model="formData.accelerationDate" dateFormat="mm/dd/yy" class="w-full"
                                showIcon />
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <p v-if="errors.constructionValue" class="text-red-500 text-sm mt-1">{{
                                    errors.constructionValue }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                ...
                                <p v-if="errors.gasDistribution" class="text-red-500 text-sm mt-1">{{
                                    errors.gasDistribution }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Projektant</label>
                                <div class="flex gap-2">
                                    <AutoComplete v-model="formData.designer" :suggestions="designerSuggestions"
                                        @complete="searchDesigners" :placeholder="'Wybierz projektanta...'"
                                        class="flex-1" @item-select="(e) => handleDesignerSelect(e.value)" dropdown
                                        forceSelection
                                    ...
                                    <PrimaryButton type="button" @click="handleNewDesigner"
                                        title="Dodaj nowego projektanta">
                                        <i class="pi pi-plus"></i>
                                    </PrimaryButton>
                                </div>
                            </div>
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Koordynator</label>
                                <div class="flex gap-2">
                                    <AutoComplete v-model="formData.coordinator" :suggestions="coordinatorSuggestions"
                                        @complete="searchCoordinators" :placeholder="'Wybierz koordynatora...'"
                                        class="flex-1" @item-select="(e) => handleCoordinatorSelect(e.value)" dropdown
                                        forceSelection
                                    ...
                                    <PrimaryButton type="button" @click="handleNewCoordinator"
                                        title="Dodaj nowego koordynatora">
                                        <i class="pi pi-plus"></i>
                                    </PrimaryButton>
                                </div>
                            </div>
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Koordynator
                                    projekt</label>
                                <div class="flex gap-2">
                                    <AutoComplete v-model="coordinatorProjectSearchQuery"
                                        :suggestions="coordinatorProjectSuggestions"
                                        @complete="searchCoordinatorProjects"
                                        :placeholder="'Wybierz koordynatora projektu...'" class="flex-1"
                                        @item-select="(e) => handleCoordinatorProjectSelect(e.value)" dropdown
                                        forceSelection
                                        :optionLabel="(coordinator: Coordinator) => `${coordinator.name} ${coordinator.lastName}`">
                                        <template #option="{ option }">
                                            <div>{{ option.name }} {{ option.lastName }}</div>
                            ...
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    <label for="pgn" class="text-gray-700 dark:text-gray-300">Prace gazoniebezpieczne
                                        (PGN)</label>
                                </div>
                            ...
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Uwagi</label>
