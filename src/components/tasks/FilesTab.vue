...
<template>
    <div class="p-6 space-y-6">
        ...
        <Card class="border border-surface-200 dark:border-surface-700 overflow-hidden">
            <template #header>
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-lg font-bold text-surface-700 dark:text-surface-300 m-0">
                            PLIKI OGÓLNE PRZYŁĄCZA
                        </h2>
                    </div>
                    <div class="flex items-center gap-3">
                        <!-- Wyszukiwarka -->
                        ...
                        <!-- Przycisk Dodaj plik -->
                        <Button label="Dodaj plik" icon="pi pi-plus" severity="warning" :disabled="isReadonly"
            ...
            <template #content>
                <div class="p-0">
                    <DataTable :value="displayedGeneralFiles" :loading="filesStore.loading" class="w-full"
                        :rowHover="true">
                        ...
                        <Column field="uploadDate" header="DATA DODANIA" style="width: 120px">
                            <template #body="{ data }">
                                <span class="text-surface-700 dark:text-surface-300">{{ formatDate(data.uploadDate)
                                }}</span>
                            </template>
                        </Column>
                        <Column field="size" header="ROZMIAR" style="width: 120px">
                            <template #body="{ data }">
                                <span class="text-surface-700 dark:text-surface-300">{{ formatFileSize(data.size)
                                }}</span>
                            </template>
                        </Column>
                        <Column field="uploadedBy" header="DODANO PRZEZ" style="width: 200px">
                            <template #body="{ data }">
                                <div class="flex items-center gap-2">
                                    <Avatar :label="getUserInitials(data.uploadedBy)" shape="circle"
                                        class="bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300" />
                                    <span class="text-surface-700 dark:text-surface-300">{{ data.uploadedBy.name
                                    }}</span>
                                </div>
                            </template>
                        </Column>
                        <Column field="description" header="OPIS" style="min-width: 200px">
                            <template #body="{ data }">
                                <span class="text-surface-600 dark:text-surface-400">{{ data.description || '-'
                                }}</span>
                            </template>
                        </Column>
                            ...
                            <template #body="{ data }">
                                <div class="relative">
                                    <Button icon="pi pi-ellipsis-v" text rounded severity="secondary"
                                        :disabled="isReadonly" @click="(e) => toggleMenu(e, data)" />
                                    <Menu :ref="(el) => setMenuRef(el, data.id)" :model="getMenuItems(data)"
                                        :popup="true" />
                                </div>
                            </template>
                        </Column>
                    ...
                    <div v-if="!showMoreGeneralFiles && filteredGeneralFiles.length > displayedGeneralFilesCount"
                        class="flex justify-center py-4 border-t border-surface-200 dark:border-surface-700">
                        <Button label="Pokaż więcej" icon="pi pi-chevron-down" text severity="secondary"
            ...
            <template #content>
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div v-for="categoryData in filesByCategory" :key="categoryData.category"
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-3">
                                    <component :is="getCategoryIcon(categoryData.category)"
                                ...
                                <Button icon="pi pi-plus" text rounded severity="secondary" :disabled="isReadonly"
                            ...
                            <div class="flex-1 p-4 space-y-3">
                                <div v-if="categoryData.files.length > 0" class="space-y-3">
                                    <div v-for="file in categoryData.files" :key="file.id"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p
                                                class="text-sm font-medium text-surface-700 dark:text-surface-300 m-0 truncate">
                                                {{ file.name }}
                                            </p>
                                        ...
                                        <div class="flex items-center gap-2 shrink-0">
                                            <Avatar :label="getUserInitials(file.uploadedBy)" shape="circle"
                                                class="bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300" />
                                            <div class="relative">
                                                <Button icon="pi pi-ellipsis-v" text rounded severity="secondary"
                                                    :disabled="isReadonly" @click="(e) => toggleMenu(e, file)" />
                                                <Menu :ref="(el) => setMenuRef(el, file.id)" :model="getMenuItems(file)"
                                                    :popup="true" />
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div v-else
                                    ...
                                    <p class="text-sm text-surface-600 dark:text-surface-400 mb-2">Brak plików w tej
                                        sekcji</p>
                                    <Button label="Dodaj pierwszy plik" text severity="secondary" :disabled="isReadonly"
                                        @click="handleAddCategorizedFile(categoryData.category)" />
                                </div>
                            </div>
                        </div>
