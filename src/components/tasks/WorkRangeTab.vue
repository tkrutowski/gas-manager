...
<template>
    <div class="p-6 space-y-6">
        ...
        <Card class="border border-surface-200 dark:border-surface-700 overflow-hidden">
            ...
            <template #content>
                <div class="space-y-4">
                    <div v-for="item in workRangeGasConnections" :key="item.id"
  ...
  <div class="mt-3">
                            ...
                            <div class="flex flex-col items-end gap-2 mt-3">
                                <div class="flex">
                                    <Button icon="pi pi-pencil" severity="secondary" text rounded :disabled="isReadonly"
                                        @click="handleEditGasConnection(item)" title="Edytuj przyłącze" />
                        ...
                        <div
                            class="grid grid-cols-6 gap-3 mt-4 pt-4 border-t border-surface-200 dark:border-surface-700">
                    ...
                    <div v-if="workRangeGasConnections.length === 0"
            ...
            <template #footer>
                <div
                    ...
                    <Button label="Dodaj zakres przyłącza" icon="pi pi-plus" :disabled="isReadonly"
        ...
        <Card class="border border-surface-200 dark:border-surface-700 overflow-hidden">
            ...
            <template #content>
                <div class="space-y-4">
                    <div v-for="item in workRangeGasStations" :key="item.id"
                        class="p-4 rounded-lg border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900">
                        <div class="grid grid-cols-6 gap-3">
                            <!-- PRZEPUSTOWOŚĆ -->
                            <div class="mt-3">
                            ...
                            <div class="flex flex-col items-end gap-2 mt-3">
                                <div class="flex">
                                    <Button icon="pi pi-pencil" severity="secondary" text rounded :disabled="isReadonly"
                                        @click="handleEditGasStation(item)" title="Edytuj punkt/stację" />
                                    <Button icon="pi pi-trash" severity="danger" text rounded :disabled="isReadonly"
                    ...
                    <div v-if="workRangeGasStations.length === 0"
                        class="p-8 text-center text-surface-600 dark:text-surface-400">
            ...
            <template #footer>
                <div
                    class="flex items-center justify-end px-4 py-3 bg-surface-50 dark:bg-surface-900 border-t border-surface-200 dark:border-surface-700">
                    <Button label="Dodaj punkt/stację" icon="pi pi-plus" :disabled="isReadonly"
                        @click="handleAddGasStation" />
                </div>
            </template>
        </Card>
        ...
        <Card class="border border-surface-200 dark:border-surface-700 overflow-hidden">
                    ...
                    <div v-if="workRangeConnection"
                        ...
                        <div class="grid grid-cols-6 gap-3">
                            ...
                            <div class="flex flex-col items-end gap-2 mt-3">
                                <div class="flex">
                                    <Button icon="pi pi-pencil" severity="secondary" text rounded :disabled="isReadonly"
                                        @click="handleEditConnection()" title="Edytuj włączenie" />
                                    <Button
                                        v-if="workRangeConnection.pressure || workRangeConnection.diameter || workRangeConnection.material || workRangeConnection.info"
                                        icon="pi pi-trash" severity="danger" text rounded :disabled="isReadonly"
                                        @click="(event) => handleDeleteConnection(event)" title="Usuń włączenie" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <template #footer>
                <div
                    class="flex items-center justify-end px-4 py-3 bg-surface-50 dark:bg-surface-900 border-t border-surface-200 dark:border-surface-700">
                    <Button :label="workRangeConnection ? 'Edytuj włączenie' : 'Dodaj włączenie'" icon="pi pi-plus"
                        :disabled="isReadonly" @click="handleEditConnection()" />
                </div>
            </template>
        </Card>

        <!-- Dialogs -->
        <WorkRangeGasConnectionDialog v-model:visible="gasConnectionDialogVisible" :item="editingGasConnection"
            :isReadonly="isReadonly" @save="handleSaveGasConnection" />

        <WorkRangeGasStationDialog v-model:visible="gasStationDialogVisible" :item="editingGasStation"
            :isReadonly="isReadonly" @save="handleSaveGasStation" />

        <WorkRangeConnectionDialog v-model:visible="connectionDialogVisible" :item="editingConnection"
            :isReadonly="isReadonly" @save="handleSaveConnection" />
    </div>
</template>
