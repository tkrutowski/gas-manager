...
function generateMockAddress(id: number): Address {
    const cities = ['Warszawa', 'Kraków', 'Gdańsk', 'Wrocław', 'Poznań', 'Łódź', 'Katowice', 'Lublin', 'Szczecin', 'Bydgoszcz'];
    const streets = ['ul. Główna', 'ul. Słoneczna', 'ul. Polna', 'ul. Leśna', 'ul. Kwiatowa', 'ul. Parkowa', 'ul. Ogrodowa', 'ul. Spacerowa'];
...
function generateMockCustomers(): Customer[] {
    ...
    const firstNames = ['Jan', 'Anna', 'Piotr', 'Maria', 'Krzysztof', 'Katarzyna', 'Tomasz', 'Magdalena', 'Paweł', 'Agnieszka', 'Marcin', 'Joanna', 'Michał', 'Ewa', 'Łukasz'];
    ...
    for (let i = 0; i < 50; i++) {
        ...
        customers.push({
            ...
            phone,
            email,
            info: Math.random() > 0.5 ? `Klient indywidualny, preferowany kontakt: ${Math.random() > 0.5 ? 'telefon' : 'email'}` : undefined,
