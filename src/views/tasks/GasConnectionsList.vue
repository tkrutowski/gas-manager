<script setup lang="ts">
  ...
  const filteredGasConnections = computed(() => {
    ...
    switch (selectedFilter.value) {
      ...
      case 'unfinished-final':
        filtered = filtered.filter(
          gc => gc.isFinished === false && gc.gasConnectionBuild?.wsgFinalAcceptanceSubmissionDate !== undefined
        );
        break;
      case 'overdue':
        ...
        filtered = filtered.filter(gc => {
          ...
          return gc.isFinished === false && finishDeadline !== undefined && compareDatesOnly(finishDeadline, today);
        });
        break;
      case 'favorites':
        const favIds = settingsStore.favoriteConnectionIds;
        filtered = filtered.filter(gc => favIds.includes(gc.id));
        break;
      case 'all':
      default:
  ...
  const sortedColumns = computed(() => {
    return [...allColumns.value].filter(col => col.visible).sort((a, b) => a.order - b.order);
  });
  ...
  const onColumnToggle = (columns: GasConnectionTableColumnConfig[]) => {
    ...
    const uniqueColumns = columns.filter((col, index, self) => index === self.findIndex(c => c.field === col.field));
      ...
      accept: () => {
        ...
        if (success) {
          ...
          gasConnections.value = rawData.map(conn => ({
            ...
            _workRangeGasConnectionsText: (
              formatWorkRangeGasConnections(conn.workRangeGasConnections) || ''
            ).toLowerCase(),
  ...
  const isSelectedFavorite = computed(() => {
    ...
    if (!row) return false;
    return settingsStore.favoriteConnectionIds.includes(row.id);
  });
  ...
  const handleToggleFavorite = () => {
    ...
    if (isSelectedFavorite.value) {
      settingsStore.removeFavoriteConnection(id);
    } else {
      settingsStore.addFavoriteConnection(id);
    }
  };
...
<template>
  <div class="flex h-screen bg-surface-50 dark:bg-surface-900 overflow-hidden">
          ...
          >
            ...
            <div class="flex gap-6 text-sm">
              <div class="flex flex-col">
                <span class="text-surface-600 dark:text-surface-400 font-medium whitespace-nowrap text-xs"
                  >Numer zadania</span
                >
                <span class="text-surface-700 dark:text-surface-300">{{ selectedRow.taskNo || '-' }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-surface-600 dark:text-surface-400 font-medium whitespace-nowrap text-xs">Klient</span>
                <span class="text-surface-700 dark:text-surface-300 whitespace-nowrap">{{
                  getPersonDisplayName(selectedRow.customer) || '-'
                }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-surface-600 dark:text-surface-400 font-medium whitespace-nowrap text-xs">Adres</span>
                <span class="text-surface-700 dark:text-surface-300 whitespace-nowrap">{{
                  formatAddress(selectedRow.address) || '-'
                }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-surface-600 dark:text-surface-400 font-medium whitespace-nowrap text-xs">Faza</span>
                <span class="text-surface-700 dark:text-surface-300 whitespace-nowrap">{{
                  formatPhase(selectedRow.phase) || '-'
                }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-surface-600 dark:text-surface-400 font-medium whitespace-nowrap text-xs">Status</span>
                <span class="text-surface-700 dark:text-surface-300 whitespace-nowrap">{{
              ...
              <template #center>
                <div class="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    :class="
                      selectedFilter === 'all'
                        ? 'bg-primary text-primary-contrast text-xs!'
                  ...
                  <Button
                    ...
                    :outlined="selectedFilter !== 'finished'"
                    severity="success"
                    :class="
                  ...
                  <SplitButton
                    ...
                    @click="handleFilterClick('unfinished')"
                    title="Niezrealizowane"
                  />
                  <Button
                    icon="pi pi-exclamation-triangle"
                    :outlined="selectedFilter !== 'overdue'"
                    severity="danger"
                    :class="
              ...
              <template #end>
                ...
                <div class="flex items-center gap-2">
                  <MultiSelect
                    ...
                    class="w-80"
                    :filter="true"
                    filterPlaceholder="Szukaj kolumn..."
                  ...
                  <Button
                    ...
                    text
                    severity="secondary"
                    @click="handleOpenColumnSettings"
                    title="Ustawienia"
                  />
                  <Button
                    icon="pi pi-refresh"
          ...
          >
            ...
            >
              ...
              <template #filter="{ filterModel, filterCallback }" v-if="col.filterable !== false">
                <InputText
                  v-if="getFilterType(col.field) === 'text'"
                  v-model="filterModel.value"
                  type="text"
                  @input="filterCallback()"
                  :placeholder="`Filtruj ${col.header.toLowerCase()}...`"
                  class="p-column-filter"
                />
                <DatePicker
                  v-else-if="getFilterType(col.field) === 'date'"
                  v-model="filterModel.value"
                ...
                />
                <InputNumber
                  v-else-if="getFilterType(col.field) === 'numeric'"
                  v-model="filterModel.value"
                  @update:modelValue="filterCallback()"
                  :placeholder="`Filtruj ${col.header.toLowerCase()}...`"
                  class="p-column-filter"
                />
                <Select
                  v-else-if="getFilterType(col.field) === 'boolean'"
                  v-model="filterModel.value"
                  :options="[
                ...
                />
                <MultiSelect
                  v-else-if="getFilterType(col.field) === 'multiselect'"
                  v-model="filterModel.value"
                  :options="getMultiSelectOptions(col.field)"
                  optionLabel="label"
                  optionValue="value"
                  @update:modelValue="filterCallback()"
                  placeholder="Dowolne"
            ...
            <template #empty>
              <div class="text-center py-8 text-surface-600 dark:text-surface-400">Brak danych do wyświetlenia</div>
            </template>
          </DataTable>
        </div>
      </div>
    ...
    <ColumnSettingsDialog
      :visible="showColumnSettingsDialog"
      :columns="allColumns"
