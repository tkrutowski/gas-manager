...
const designProgress = computed(() => {
    ...
    const withApproval = designProjects.value.filter(
        gc => gc.gasConnectionDesign.zudpReceiptDate !== undefined
    ).length;
...
const designAwaitingApproval = computed(() => {
    ...
    return designProjects.value.filter(
        gc =>
            gc.gasConnectionDesign.zudpSubmissionDate !== undefined &&
            gc.gasConnectionDesign.zudpReceiptDate === undefined
    ).length;
});
