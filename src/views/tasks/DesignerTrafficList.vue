...
const selectedItem = computed<DesignerTraffic | null>(() => {
  ...
  return (
    filteredItems.value.find(d => d.id === selectedDesignerTrafficId.value) ?? null
  );
});
...
<template>
  <div class="flex h-screen bg-surface-0 dark:bg-surface-950 overflow-hidden">
                ...
                <div
                  ...
                  <div class="flex items-center gap-1.5">
                    <button type="button"
                      ...
                      ]" :disabled="!item.phone" @click.stop="handleCall(item)">
                      <PhoneIcon class="w-4 h-4" />
                    </button>
                    <button type="button"
                      class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-xs font-medium transition-colors"
                      :class="[
                        item.email
                          ...
                          : 'bg-surface-200 dark:bg-surface-800 text-surface-400 cursor-not-allowed',
                      ]" :disabled="!item.email" @click.stop="handleEmail(item)">
                      <EnvelopeIcon class="w-4 h-4" />
                    </button>
                  </div>
                </div>
    ...
    <DesignerTrafficFormDialog v-model:visible="showDialog" :designer-traffic="designerTrafficForEdit"
      @close="handleDialogClose" @designer-traffic-added="handleDesignerTrafficAdded"
