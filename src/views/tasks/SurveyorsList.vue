...
const selectedSurveyor = computed<Surveyor | null>(() => {
  ...
  return (
    filteredSurveyors.value.find(s => s.id === selectedSurveyorId.value) ?? null
  );
});
...
const handleFilterChange = (filter: SurveyorFilter) => {
  ...
  if (
    ...
    !filteredSurveyors.value.some(s => s.id === selectedSurveyorId.value)
  ) {
    selectedSurveyorId.value = null;
  }
};
...
<template>
  <div class="flex h-screen bg-surface-0 dark:bg-surface-950 overflow-hidden">
        ...
        <div class="bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-4">
          <div class="flex items-center justify-between mb-4">
            ...
            <p class="text-xs text-surface-500 dark:text-surface-400">
              Kliknij kartę, aby zaznaczyć geodetę.
            </p>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            <button v-for="surveyor in filteredSurveyors" :key="surveyor.id" type="button"
                ...
                <div
                  ...
                  <div class="flex items-center gap-1.5">
                    <button type="button"
                      ...
                      ]" :disabled="!surveyor.phone" @click.stop="handleCall(surveyor)">
                      <PhoneIcon class="w-4 h-4" />
                    </button>
                    <button type="button"
                      class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-xs font-medium transition-colors"
                      :class="[
                        surveyor.email
                          ...
                          : 'bg-surface-200 dark:bg-surface-800 text-surface-400 cursor-not-allowed',
                      ]" :disabled="!surveyor.email" @click.stop="handleEmail(surveyor)">
                      <EnvelopeIcon class="w-4 h-4" />
                    </button>
                  </div>
                </div>
    ...
    <SurveyorFormDialog v-model:visible="showDialog" :surveyor="surveyorForEdit" @close="handleDialogClose"
      @surveyor-added="handleSurveyorAdded" @surveyor-updated="handleSurveyorUpdated" />
  </div>
</template>
